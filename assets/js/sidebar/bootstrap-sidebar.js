/* eslint no-undef:0,space-before-function-paren:0,eqeqeq:0  */
$(function() {
  // $('.dropdown-toggle.open').click();// デフォルトで開く
  // サイドバーの表示・非表示を切り替えるイベントを設定する
  $('.sidebar-ctrl,.sidebar-window-ctrl > .btn').on('click mouseover', function(
    _event
  ) {
  ...
  $('.sidebar-content,.sidebar:not([class*=floating])').on(
    'mouseover',
    function() {
      if (window.matchMedia('(max-width:576px)').matches) {
        /* 576px以下 xs */
